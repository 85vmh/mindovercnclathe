...
class LatheToolsRepositoryImpl : LatheToolsRepository {
    ...
    override fun getUnmountedLatheTools(): List<LatheTool> {
        return transaction {
            val toolHolderQuery = ToolHolderTable.slice(ToolHolderTable.cutterId).select(ToolHolderTable.cutterId.isNotNull())
    ...
    private fun LatheToolEntity.toLatheTool(): LatheTool? {
        return when (type) {
            ...
            ToolType.Parting -> LatheTool.Parting(
                toolId = id.value, insert = insert!!.toCuttingInsert(), bladeWidth = bladeWidth!!, maxXDepth = maxXDepth ?: 0.0
            )
            ...
            ToolType.OdThreading -> LatheTool.OdThreading(
                toolId = id.value, insert = insert!!.toCuttingInsert(), minPitch = minThreadPitch ?: 0.0, maxPitch = maxThreadPitch ?: 0.0
            )
