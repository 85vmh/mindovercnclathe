...
data class HandWheelsUiModel(val active: Boolean, val increment: Float, val units: String = "mm")

data class WcsUiModel(val activeOffset: String, val wcsOffsets: List<WcsOffset>) {
  val selected = wcsOffsets.firstOrNull { it.coordinateSystem == activeOffset }
}

data class WcsOffset(val coordinateSystem: String, val xOffset: Double, val zOffset: Double)

data class SimpleCycleUiModel(val simpleCycleParameters: SimpleCycleParameters)
