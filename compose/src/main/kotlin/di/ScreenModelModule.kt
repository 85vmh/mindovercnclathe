...
val ScreenModelModule = DI.Module("ScreenModel") {
    ...
    bindProvider {
        TurningSettingsScreenModel(instance())
    }

    bindProvider {
        TaperSettingsScreenModel(instance())
    }

    bindProvider {
        VirtualLimitsScreenModel(instance(), instance())
    }
    ...
    bindProvider {
        StatusRootScreenModel(instance())
    }

    bindProvider {
        ProgramsRootScreenModel(instance())
    }
...
fun simpleCycleModule(simpleCycle: SimpleCycle) = DI.Module("simpleCycle") {
    bindProvider("simpleCycle") { simpleCycle }
}