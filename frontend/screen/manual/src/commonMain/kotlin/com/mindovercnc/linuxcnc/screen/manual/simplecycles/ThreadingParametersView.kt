...
fun ThreadingParametersView(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        DropDownSetting(
            ...
            onValueChanged = { selectedValue ->
                SimpleCycleParameters.ThreadingParameters.ThreadType.entries
                    ...
                    ?.let { viewModel.setThreadType(it) }
            }
        )

        CycleParameter(
            parameterLabel =
                when (parametersState.threadType) {
        ...
        CycleParameter(
            ...
            onValueChange = { viewModel.setThreadSpringPasses(it.toInt()) },
        )
    }
}
    ...
    Row(modifier = modifier) {
        Row(
            Modifier.width(200.dp)
                .selectable(selected = isExternalThread, onClick = { onTypeChanged(true) }),
            horizontalArrangement = Arrangement.Start,
            verticalAlignment = Alignment.CenterVertically
        ) {
            RadioButton(selected = isExternalThread, onClick = { onTypeChanged(true) })
            Text(text = "External Thread")
        }
        Row(
            Modifier.width(200.dp)
                .selectable(selected = isExternalThread.not(), onClick = { onTypeChanged(false) }),
            horizontalArrangement = Arrangement.Start,
            verticalAlignment = Alignment.CenterVertically
        ) {
            RadioButton(selected = isExternalThread.not(), onClick = { onTypeChanged(false) })
